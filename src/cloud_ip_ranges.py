...
class CloudIPRanges:
    sources: ClassVar[dict[str, list[str]]] = {
        ...
        "gcore_cdn": ["https://api.gcore.com/cdn/public-net-list"],
        "gcore_cloud": ["https://api.gcore.com/cloud/public/v1/ipranges/egress"],
    ...
    def _enforce_max_delta(self, old: dict[str, Any], new: dict[str, Any], *, max_ratio: float, source_key: str) -> None:
        ...
        v4_fail = r4 != float("inf") and r4 > effective_max_ratio and abs4 > _MIN_ABS_CHANGE and s["ipv4"]["new_addrs"] < s["ipv4"]["old_addrs"]
        v6_fail = r6 != float("inf") and r6 > effective_max_ratio and abs6 > _MIN_ABS_CHANGE and s["ipv6"]["new_addrs"] < s["ipv6"]["old_addrs"]
